...
The original challenge `README.md`, `main.go`, and `matrix.csv` have been preserved in the `LeagueBackendChallenge` directory.
## Testing
...
Empty csv - this should be considered valid; and will either print nothing to output (matches provided echo function), or return 0 for sum/multiply endpoints
...
Large csvs - An option to limit the request body size has been commented out in the parseMatrix function, in consideration of potential file/bandwidth processing limits. If processing a document that goes beyond this threshold, an appropriate processing error will be returned. This can be verified by un-commenting the option to limit maximum csv size in the parseMatrix function

## Additional Notes
The original specification is included in the `LeagueBackendChallenge` directory.